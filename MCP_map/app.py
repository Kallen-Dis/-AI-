...
架构:
  前端 (百度地图 JS) ←→ Flask API ←→ Multi-agent Orchestrator
                                   ←→ 百度 Web 服务 (路线polyline / POI搜索)
"""
...
# Windows 控制台默认 GBK，打印 ¥ 等字符会直接让规划失败
...
from tools.baidu_tools import BaiduTools

BAIDU_AK, BAIDU_BROWSER_AK = load_baidu_keys()
_map_tools = BaiduTools(api_key=BAIDU_AK) if BAIDU_AK else None
...
def _route_with_polyline(o_lng, o_lat, d_lng, d_lat, mode="driving", city=""):
    """两点路线规划, 返回含 polyline 的结果（百度 Direction Lite）"""
    if not _map_tools:
        return {
            "distance": 0,
            "duration": 0,
            "polyline": [[o_lng, o_lat], [d_lng, d_lat]],
            "error": "BAIDU_MAP_AK missing",
        }
    result = _map_tools.route_with_polyline(o_lng, o_lat, d_lng, d_lat, mode=mode, city=city)
...
@app.route("/")
def index():
    html = render_template("index.html", baidu_browser_ak=BAIDU_BROWSER_AK)
...
# ================================================================
#  路线规划 API (百度 Direction Lite, 含 polyline)
# ================================================================
...
def api_search_place():
    ...
    if not kw:
        return jsonify([])
    if not _map_tools:
        return jsonify({"error": "BAIDU_MAP_AK missing"}), 500
