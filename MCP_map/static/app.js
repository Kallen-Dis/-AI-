...
function createOverlayGroup() {
    ...
    return {
        addLayer: function (overlay) {
            items.push(overlay);
            if (visible && map) map.addOverlay(overlay);
        },
        clearLayers: function () {
            items.forEach(function (overlay) { if (map) map.removeOverlay(overlay); });
            items = [];
        },
        setVisible: function (show) {
            visible = show;
            items.forEach(function (overlay) {
                if (!map) return;
                if (show) map.addOverlay(overlay);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    map.centerAndZoom(new BMap.Point(117.05, 30.53), 12);
    map.enableScrollWheelZoom(true);
    map.addControl(new BMap.NavigationControl({ anchor: BMAP_ANCHOR_TOP_RIGHT }));
    map.addControl(new BMap.ScaleControl({ anchor: BMAP_ANCHOR_BOTTOM_LEFT }));

    markerLayerGroups.attraction = createOverlayGroup();
    markerLayerGroups.restaurant = createOverlayGroup();
...
var GUIDE_STORAGE_KEY = 'travel_guide_hide';
function initGuide() {
    setTimeout(function () {
...
function openGuide() {
    var el = document.getElementById('guideDialog');
    if (el) el.style.display = 'flex';
}
function closeGuide() {
    var box = document.getElementById('guideDontShow');
    if (box && box.checked) localStorage.setItem(GUIDE_STORAGE_KEY, '1');
    var el = document.getElementById('guideDialog');
...
function switchTab(tab) {
    ...
    if (tab === 'map') setTimeout(function () { map && map.checkResize && map.checkResize(); }, 100);
}
function toggleSidebar() {
    document.getElementById('sidebar').classList.toggle('collapsed');
    setTimeout(function () { map && map.checkResize && map.checkResize(); }, 350);
}
...
function placeMarkers(items, type) {
    items.forEach(function (w, i) {
        ...
        marker.addEventListener('click', function () { showPopup(marker, w, type); });
        if (type === 'attraction') {
            marker.addEventListener('dragend', function (e) {
                w.lng = e.point.lng; w.lat = e.point.lat;
...
function showPopup(marker, w, type) {
    ...
    var info = new BMap.InfoWindow(h, { width: 280 });
...
function fitView() {
    ...
    ['attraction', 'restaurant', 'hotel'].forEach(function (t) {
        markerGroups[t].forEach(function (item) {
            if (item.marker && item.marker.getPosition) points.push(item.marker.getPosition());
        });
    });
...
function drawRoutes(routes) {
    ...
    routes.forEach(function (r, i) {
        ...
        var pts = pl.map(function (p) { return new BMap.Point(p[0], p[1]); });
        console.log('[ROUTE DEBUG] Drawing polyline with', pts.length, 'coordinates');
        ...
        if (mid) {
            ...
            var labelMarker = new BMap.Label(
                '<div class="route-label" style="border-color:' + color + ';color:' + color + ';">' + km + 'km / ' + min + 'min</div>',
                { position: new BMap.Point(mid[0], mid[1]), offset: new BMap.Size(-40, -10) }
            );
            labelMarker.setStyle({ border: 'none', background: 'transparent' });
...
function drawDashed() {
    ...
    for (var i = 0; i < waypoints.length - 1; i++) {
        ...
        var line = new BMap.Polyline(
            ...
            { strokeColor: c, strokeWeight: 3, strokeOpacity: 0.6, strokeStyle: 'dashed' }
        );
        routeLayerGroup.addLayer(line);
        routePolylines.push(line);
    }
...
function planRoutes() {
    ...
    fetch('/api/route_plan_multi', {
        ...
        data.segments.forEach(function (seg, i) {
            ...
            if (pl.length >= 2) {
                var pts = pl.map(function (p) { return new BMap.Point(p[0], p[1]); });
                var line = new BMap.Polyline(pts, { strokeColor: c, strokeWeight: 5, strokeOpacity: 0.85 });
                routeLayerGroup.addLayer(line);
                routePolylines.push(line);
                var mid = pl[Math.floor(pl.length / 2)];
                if (mid) {
                    var labelMarker = new BMap.Label(
                        '<div class="route-label" style="border-color:' + c + ';color:' + c + ';">' +
...
function locJEntry(id) {
    ...
    if (e && e.lng && e.lat) { switchTab('map'); setTimeout(function () { map.centerAndZoom(new BMap.Point(e.lng, e.lat), 16); }, 300); }
}
    ...
    setTimeout(function () {
        ...
        for (var i = 0; i < group.length; i++) {
            if (group[i].data.name === name) {
                showPopup(group[i].marker, group[i].data, type);
                break;
            }
        }
    }, 150);
